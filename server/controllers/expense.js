...
const createExpense = async (req, res) => {
    try{
        ...
        const query = "INSERT INTO expense (user_id, type, category, amount, date) VALUES (?,?,?,?,?)";
        connection.query(query, [user_id, type, category, amount, date], (err, result) => {
            if (err) {
                return response.status(500).json({ error: "Internal server error" });
              }
...
const updateExpense = async (req, res) => {
  try{
      ...
      const query = "UPDATE expense set type = ?, category = ?, amount = ?, date = ? where id = ?";
      connection.query(query, [type, category, amount, date, id], (err, result) => {
          if (err) {
              return response.status(500).json({ error: "Internal server error" });
            }
